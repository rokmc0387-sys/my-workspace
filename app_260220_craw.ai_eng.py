...
def is_valid_sentence(text, irrelevant_keywords):
    ...
    # 2. 단순 문장 (50자 미만)
    if len(text) < 50: return False, "50자 미만 (단순 문장)"
        
    # 3. 단순 질문 (70자 미만 & ?)
    if len(text) < 70 and '?' in text: return False, "70자 미만 단순 질문"
...
with tab1:
    ...
    if crawl_source == "YouTube (API)":
        ...
        video_url_or_id = st.text_input("YouTube 동영상 URL 또는 ID", placeholder="[https://www.youtube.com/watch?v=](https://www.youtube.com/watch?v=)... 또는 VideoID")
    ...
    # 4. Samsung.com
        ...
        target_url = st.text_input("대상 URL (Samsung.com)", placeholder="[https://www.samsung.com/sec/](https://www.samsung.com/sec/)...")
    ...
    # 5. LGE.co.kr
        ...
        target_url = st.text_input("대상 URL (LGE.co.kr)", placeholder="[https://www.lge.co.kr/](https://www.lge.co.kr/)...")
...
with tab3:
    ...
    if df_target is not None:
        ...
        if 'level1_recommended' not in st.session_state:
            st.session_state['level1_recommended'] = "Picture Quality, Design, Software/UX, Price, Durability, Sound, Connectivity, Setup, Gaming, Performance, Energy, Portability, Others"

        col_cat1, col_cat2 = st.columns([4, 1])
        with col_cat1:
            level1_cats = st.text_area("Level 1 Categories (Comma separated)", 
                                       value=st.session_state['level1_recommended'],
                                       help="Top-level categories for extraction. You can edit manually or use AI recommendation.")
        with col_cat2:
            st.write("") # For spacing
            st.write("")
            if st.button("🪄 AI Suggest", help="Analyze collected data to suggest optimal categories."):
                if not api_key:
                    ...
                    st.error("No collected or cleaned data found.")
                else:
                    with st.spinner("Analyzing data..."):
                        ...
                        Analyze these reviews and suggest the most suitable 10-15 top-level categories (Aspect Level 1) for classifying the entire dataset.
                        
                        [Review Samples]
                        {combined_samples}
                        
                        [Output Format]
                        - Output ONLY the category names separated by commas in a single line.
                        - Example: Picture, Sound, Design, Price, Performance, Features, Shipping, Others
                        - Ensure the categories are clear and non-overlapping.
                        ...
                        try:
                            ...
                            # Remove unnecessary quotes/markdown
...
with tab6:
    ...
    power_bi_url = st.text_input("Power BI 게시(Publish) URL 입력", value=saved_url, placeholder="[https://app.powerbi.com/view?r=](https://app.powerbi.com/view?r=)...")
